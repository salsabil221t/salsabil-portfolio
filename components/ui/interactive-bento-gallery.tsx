    ...
    <>
      ...
      >
        ...
        <div className="h-full flex flex-col">
          <div className="flex-1 p-2 sm:p-3 md:p-4 flex items-center justify-center bg-background/60">
                ...
                >
                  ...
                  {selectedItem.tags && selectedItem.tags.length > 0 ? (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {selectedItem.tags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full bg-white/15 px-2.5 py-0.5 text-[10px] sm:text-xs font-semibold text-white/90"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  {selectedItem.link && selectedItem.link !== "#" ? (
                    <a
                      ...
                      className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-white/90 px-4 py-1.5
                                text-xs sm:text-sm font-semibold text-gray-900 transition-transform hover:scale-105"
                    >
...
// BentoTile renders one grid tile. When `assembleProgress` is provided the
// tile pops in oversized and settles into its slot as the value crosses its
// band; otherwise it uses the original mount animation.
const BentoTile = ({
  item,
  index,
  total,
  ...
  const progress = assembleProgress ?? fallback;
  const start = 0.02 + (index / Math.max(1, total)) * 0.9;
  const duration = 0.9 / Math.max(1, total);
  ...
  const popScale = useTransform(
    progress,
    [start + duration * 0.3, start + duration],
    [1.6, 1]
  );

  const scrollDriven = Boolean(assembleProgress);
  ...
  return (
    <motion.div
      ...
      style={
        scrollDriven ? { opacity: popOpacity, scale: popScale } : undefined
      }
      initial={scrollDriven ? false : "hidden"}
      animate={scrollDriven ? undefined : "visible"}
      variants={
        scrollDriven
          ? undefined
          : {
      ...
      whileHover={scrollDriven ? undefined : { scale: 1.02 }}
      drag
...
interface InteractiveBentoGalleryProps {
  ...
  description: string;
  linkLabel?: string;
  /** 0→1 scroll progress: tiles pop in and snap into place one by one. */
  assembleProgress?: MotionValue<number>;
}
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({
  ...
  // Keep local order in sync when content changes (e.g. after a dashboard save)
  useEffect(() => {
    setItems(mediaItems);
  }, [mediaItems]);
              ...
              <BentoTile
                ...
                onDragEnd={(idx, info) => {
                  ...
                  if (Math.abs(moveDistance) > 50) {
                    ...
                    const draggedItem = newItems[idx];
                    const targetIndex =
                      moveDistance > 0
